...
class IncorrectTeethFinder
{
     ...
     */
    private function findMultipleServices() : StructuredResponse{
     ...
     * @return StructuredResponse
     */
    private function findTeethCodesAndDiagnosis(StructuredResponse $response) : StructuredResponse{
        if ($response->body !== []){
     ...
     * @return StructuredResponse
     */
    private function findSimultaneousCasesIDs(StructuredResponse $response): StructuredResponse{
        if ($response->body !== []){
            $simultaneous = [];
    ...
    private function findIncorrectTeethCodeInclusion() : StructuredResponse{
        $query = ("SELECT * FROM stom_xml_pm_sl_stom
     ...
     * @return StructuredResponse
     */
    private function findRequiredTeethCode() : StructuredResponse{
        $query = ("SELECT * FROM stom_xml_pm_sl_stom
                   INNER JOIN stom_xml_hm_zsl ON stom_xml_hm_zsl_idcase = stom_xml_pm_sl_stom_sl_idcase
                   INNER JOIN stom_xml_lm ON stom_xml_lm__id_pac = stom_xml_hm_zsl_id_pac
                    ");
     ...
     * @return StructuredResponse
     */
    private function findSimultaneousTeethInclusion(StructuredResponse $response) : StructuredResponse{
        if ($response->body !== []){
    ...
    public function findIncorrectTeeth() : array{
        $incorrectTeethCodeInclusion = $this->findIncorrectTeethCodeInclusion();
        $incorrectRequiredTeeth = $this->findRequiredTeethCode();
        $services = $this->findMultipleServices();
        $codesAndDiagnosis = $this->findTeethCodesAndDiagnosis($services);
        $casesIDs = $this->findSimultaneousCasesIDs($codesAndDiagnosis);
        $simultaneousTeethInclusion = $this->findSimultaneousTeethInclusion($casesIDs);
        return [
