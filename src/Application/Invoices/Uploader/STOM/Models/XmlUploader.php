...
class XmlUploader
{
    ...
    private function lmUpload(array $lmPers){
        try{
            ...
            foreach ($lmPers AS $row) {
                ...
                $ot = "'".$row['OT']."'";
                $gender = "'".$row['W']."'";
                $dateBirth = (int) strtotime($row['DR']);
                $SNILS = "'".$row['SNILS']."'";
                if(isset($row['OKATOG'])){$OKATOG ="'".$row['OKATOG']."'"; } else {$OKATOG = 'null';}
                if(isset($row['OKATOP'])){$OKATOP ="'".$row['OKATOP']."'"; } else {$OKATOP = 'null';}
                $query .= (" ($idPac, $fam, $im, $ot,  $gender, $dateBirth, $SNILS, $OKATOG, $OKATOP),");
            };
    ...
    public function xmlDataToMySQLData(array $data){
        $result = [];
        if($this->lmUpload($data['LM'])){
            $result['lmMessage'] = 'LM залит';
        }
        if($this->pmUpload($data['PM'])){
            $result['pmMessage'] = 'PM залит';
        }
        if ($this->hmUpload($data['HM'])){
            $result['hmMessage'] = 'HM залит';
        }
        return $result;
    }
